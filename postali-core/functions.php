 ...
 */
	...
	function checkWebpCompatibility($image_url) {
		...
		$browser = "Other";
		$webp_status = get_headers($image_url . '.webp', 1)[0];
		...
		if( $browser == 'Internet Explorer' || $browser == 'Other' || str_contains($webp_status, '200')  !== true ) {
			return $image_url;
		} else {
    ...
    function klf_acf_input_admin_footer() { ?>
        ...
        args.palettes = ['#ffffff', '#000000','#1a6b8d','#AED69E','#213D49']
    ...
    function postali_register_acf_blocks() {
        ...
        wp_enqueue_style( 'blocks-styles', '/wp-content/themes/postali-core/blocks/assets/css/styles.css' ); // Enqueue Child theme style sheet (theme info)
        wp_register_script('slick-scripts', '/wp-content/themes/postali-core/blocks/assets/scripts/slick-custom.min.js',array('jquery'), null, true); 
		wp_enqueue_script('slick-scripts');
